package com.quintus.labs.datingapp.mediapicker.utils;
...
public class MediaUtils {
    ...
    public static Uri getMediaItemUriFromPath(Context context, String path) {
        ...
        Uri imageUri = null;
        if (null == cursor) {
            Log.d("Tag", "Null cursor for file " + path);
            return imageUri;
        }
        long id = 0;
        if (cursor.getCount() > 0) {
            ...
            imageUri = Uri.parse("content://media/external/images/media/" + id);
        }
        cursor.close();
        return imageUri;
    }
    ...
    public static String getPathFromBitpam(Bitmap mBitmap){
        String selectedImagePath="";
        ByteArrayOutputStream bao = new ByteArrayOutputStream();
        try {

            mBitmap.compress(Bitmap.CompressFormat.JPEG, 70, bao);
            byte de[] = bao.toByteArray();

            mBitmap = BitmapFactory.decodeByteArray(de, 0, de.length);

            File f = new File(Environment.getExternalStorageDirectory()
                    + File.separator + "myu.jpg");
            f.createNewFile();
            FileOutputStream fo = new FileOutputStream(f);
            fo.write(de);

            fo.close();
           return selectedImagePath = f.getPath();
