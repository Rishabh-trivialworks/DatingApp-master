...
import com.quintus.labs.datingapp.chatview.utils.DownloadFileUtils;
...
public class MyXMPP implements PingFailedListener, DownloadListener {
    ...
    public void blockUnblock(final int userId, final boolean block) {
        ...
        AsyncTask<Void, Void, Void> execute = new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... voids) {
                ...
                if (block) {
                    XMPPAddNewPrivacyList(connection,userId+"",block);
                } else {
                    XMPPAddNewPrivacyList(connection,userId+"",block);
...
//            downloadMediaMessage(messageModel,temp1[0]);
    ...
    private void downloadMediaMessage(MessageModel messageModel,String tableName){
        MessageBody body = ChatUtils.getMessageBody(messageModel.getMsg());
        switch (ChatUtils.MESSAGE_TYPE_TEXT){
            case ChatUtils.MESSAGE_TYPE_TEXT:
                ChatActivity.addLeftMessage(messageModel);
                CommonMethods commonMethods = new CommonMethods(context);
                commonMethods.createTable(tableName);
                commonMethods.insertIntoTable(messageModel.getSender(), messageModel.getSender(), messageModel.getReceiver(), messageModel.getMsg(), "r", "TEXT",messageModel.getTime());
                break;
            default:
               String dirPath = Utils.getRootDirPath(context);
               String fileName = Utils.getFilename(body.getLocalUrls().get(0));
               DownloadFileUtils.downloadFile(body.getMediaUrl().get(0), dirPath, fileName, new DownloadListener() {
                   @Override
                   public void onDownLoadSuccess(String filePath) {
                       List<String> listLocal = new ArrayList<>();
                       listLocal.add(filePath);
                       body.setLocalUrls(listLocal);
                       messageModel.setMsg(ChatUtils.getStringFromMessageBody(body));
                       ChatActivity.addLeftMessage(messageModel);
                       CommonMethods commonMethods = new CommonMethods(context);
                       commonMethods.createTable(tableName);
                       commonMethods.insertIntoTable(messageModel.getSender(), messageModel.getSender(), messageModel.getReceiver(), messageModel.getMsg(), "r", "TEXT",messageModel.getTime());
